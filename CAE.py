...
# tf Graph input (only pictures)
        ...
        if is_train is True:
            if param_file is True:
                self.saver.restore(self.sess, "./params/CAE_.ckpt")
    ...
    def buildautoencoder(self):
            ...
            with tf.variable_scope('decoder'):
                ...
                with tf.variable_scope('deconv_l1'):
                    ...
                    h_d_conv1 = tf.nn.relu(self.deconv2d(self.encoder, self.wd_conv1, output_shape_d_conv1))
                ...
                with tf.variable_scope('loss'):
                    ...
                    Wset = [self.we_conv1, self.we_conv2, self.wd_conv1, self.wd_conv2]
    ...
    def learn(self):
            ...
            self.saver.save(self.sess, "./params/CAE_.ckpt")
    ...
    def fine_tuning(self):
        ...
        fine-tuning
        """
        x = tf.placeholder(tf.float32, shape=[None, 784], name='image_fine-tuning') / 255.
        ...
        encoder = tf.reshape(conv2, [-1, 7 * 7 * 32])
        w_fc1 = tf.get_variable('wfc1', [7 * 7 * 32, 1024], initializer=w_initializer)
        b_fc1 = tf.get_variable('bfc1', [1024], initializer=b_initializer)
        result = tf.nn.relu(tf.matmul(encoder, w_fc1) + b_fc1)

        w_fc2 = tf.get_variable('wfc2', [1024, 10], initializer=w_initializer)
        b_fc2 = tf.get_variable('bfc2', [10], initializer=b_initializer)
        result2 = tf.nn.softmax(tf.matmul(result, w_fc2) + b_fc2)
        print (result2.shape)

        cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits( labels=y,logits=result2))
        loss = tf.reduce_mean(-tf.reduce_sum(y * tf.log(result2), reduction_indices=[1]))

        train_op = tf.train.AdamOptimizer(0.001).minimize(loss)
        correct_pred = tf.equal(tf.argmax(result2, 1), tf.argmax(y, 1))
